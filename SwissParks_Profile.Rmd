...
area <- area %>%
  group_by(PARK) %>%
  summarise(km2 = round(sum(km2),2))

#The total area of parks:
area %>%
  summarize(area = sum(km2))#km2
...
```{r echo=TRUE}
general <- data.frame("Parks"= area$PARK, "km2"= area$km2, "Photos"= photos$photo, "Photos_per"= photos$ratio_photos, "Photos_km"=round((photos$photo/area$km2),2),"Users"=users_flickr$user, "Users_per"=users_flickr$ratio_users, "Users_km"=round((users_flickr$user/area$km2),2))
