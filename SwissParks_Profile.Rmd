...
#The proportion of user per park
users_flickr <- users_flickr %>%
  group_by(PARK) %>%
  summarise(user = n())%>%
  mutate(ratio_users= round((user / sum(user))*100, 1)) %>% 
...
```{r echo=FALSE}
general <- data.frame("Parks"= area$PARK, "Area(km2)"= area$km, "#Flickr"= photos$photo, "%Flickr"= photos$ratio_photos, "Flickr/km2"=round((photos$photo/area$km),2),"#Users"=users_flickr$user, "%Users"=users_flickr$ratio_users, "Users/km2"=round((users_flickr$user/area$km),2), 
                      check.names=FALSE)
...
```{r echo=FALSE}
#Values graph of number of users per season
...
sp<- pivot_wider(season_num, names_from = SEASON, values_from = user)
season_park <- data.frame(sp)

  #Stacked normalized horizontal bar graph
sp %>% pivot_longer(-PARK) %>% 
  mutate(name = factor(name, levels = c("AUTUMN", "SUMMER", "SPRING", "WINTER"))) %>%
  ggplot(aes(x = factor(PARK), y = value, fill=name, group=name))+
  geom_bar(stat='identity', color='white', position='fill')+
  scale_y_continuous(labels = scales::percent)+
  scale_fill_manual(values=c("#ed7c31", "#ffbf00", "#00b04f", "#00b0f0"))+
...
*Percentage table of Users according season in Parks*
...
season_per <- subset(season_uper, select=-c(user))

sp_per<- pivot_wider(season_per, names_from = SEASON, values_from = ratio_users)
season_park_per <- data.frame(sp_per)
...
canton_users <- canton_users[order(canton_users$Total_Canton, decreasing = TRUE),]
...
park_users <- park_users[order(park_users$Total_Park, decreasing = TRUE),]
...
```
*Head plot of frequencies of words*
```{r echo=FALSE}
Heaps_plot(dtm)

```

*Zipf law plot of frequencies of words*
```{r echo=FALSE}
Zipf_plot(dtm)
